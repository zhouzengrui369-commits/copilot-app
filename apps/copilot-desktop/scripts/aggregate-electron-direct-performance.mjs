...
import { aggregateDirectPerformanceRuns } from './direct-performance-config.mjs';

const OUTPUT_BASENAME = 'performance-aggregate-r22.json';
const RAW_BASENAMES = Object.freeze([
...
async function composeAggregate(args) {
  ...
  if (
    path.basename(outputPath) !== OUTPUT_BASENAME
    || rawPaths.some((filePath, index) => path.basename(filePath) !== RAW_BASENAMES[index])
  ) {
  ...
  const parentStat = await safeLstat(lexicalParent, 'BLOCKED_DIRECT_PERF_AGGREGATE_CLI_DIRECTORY');
  ...
  const records = await Promise.all(rawPaths.map((filePath) => readPrivateRaw(filePath)));
  const pairs = records.map((record, index) => ({ raw: RAW_BASENAMES[index], record }));
  const aggregate = aggregateDirectPerformanceRuns(pairs);
...
async function readPrivateRaw(filePath) {
