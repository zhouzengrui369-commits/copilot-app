...
async function main() {
  ...
  if (candidateBindingConfig.candidate !== CONTRACT.candidate) {
    throw blocker('BLOCKED_DIRECT_PERF_R31_CANDIDATE_REQUIRED');
  }
  ...
  try {
    ...
    const runBinding = createCandidateBoundDirectPerformanceRunBinding(CONTRACT, {
      challenge,
    ...
    if (candidateBinding.candidate !== CONTRACT.candidate) {
      throw blocker('BLOCKED_DIRECT_PERF_R31_CANDIDATE_REQUIRED');
    }
    ...
    resultToPublish = {
      ...
      runtimeProbe: {
        ...
        metricsObservedAfterGateMs: round(
          metricsObservation.observedAtMs - readyObservation.observedAtMs,
        ),
      },
