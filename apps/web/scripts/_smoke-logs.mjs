...
page.on("console", (msg) => {
  const text = msg.text();
  if (text.includes("KnowledgeGraph3D") || text.includes("fly") || text.includes("click hit")) {
...
const state = await page.evaluate(() => {
  ...
  return {
    camera: { x: d.camera.position.x, y: d.camera.position.y, z: d.camera.position.z },
    target: { x: d.controls.target.x, y: d.controls.target.y, z: d.controls.target.z },
    selectedNodeId: d.selectedNodeId(),
    positionsSize: d.positions.size,
  };
});
